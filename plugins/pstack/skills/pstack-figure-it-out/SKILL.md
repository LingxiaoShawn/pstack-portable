...
- For consequential unresolved design decisions, request a design-only result from **architect**. Use **arena** only when comparing distinct candidates would help resolve the choice. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
