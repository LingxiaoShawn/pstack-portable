...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from the host-exposed current-project transcript, if available. Don't glob across `unrelated project transcript directories`. That reads unrelated private chats. Walk the log against what actually happened:
