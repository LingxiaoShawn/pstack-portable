...
- **Brand semantic primitives.** `UserId` and `OrderId` are strings underneath but should not be interchangeable. Newtypes in Rust, opaque types in Swift, value classes in Kotlin, phantom types in Haskell, branded intersections in TypeScript. Validate at creation and reuse that guarantee while it remains valid. Mutation, deserialization, casts and foreign interfaces can invalidate it; preserve checks where those guarantees change.
...
- **Strengthen a type only where partiality appears.** A runtime assertion or null check can reveal a representable invalid state; strengthen the type when that actually rules the state out. If absence is legitimate or a guarantee depends on runtime input, mutation or concurrency, keep the relevant check. Then stop. The type system's job is to track the cases each use site must handle, not to describe the data as precisely as possible. Prefer total functions. `sum` of an empty list is 0, so it takes the plain list. `head` of an empty list has no answer, so it demands the non-empty one.
...
- "Where did this `any`, this `as`, this `assertNotNull` come from?" Trace the guarantee it relies on. Validate or narrow where that guarantee can actually be established; keep necessary checks for later state changes.
