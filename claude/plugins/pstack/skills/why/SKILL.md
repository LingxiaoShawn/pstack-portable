...
Before spawning investigators, list the available MCPs advertised by the current host. Use the available-tools map when present. Otherwise use the available tool descriptions; do not guess a tool metadata directory.
...
- `subagent_type`: the host's general-purpose worker role
